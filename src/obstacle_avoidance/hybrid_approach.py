...
class ObstacleAvoidanceController:
    ...
    def __init__(self, initial_altitude=15):
        ...
        self.min_possible_range_down_to_descend = 6
        ...
        for i, dist_val in enumerate(lidar_ranges):
            ...
            if math.cos(a_diff) > 0:
                ...
                if lateral_offset < self.safety_box_half:
                    ...
                    min_distance = min(obstacle_distance, min_distance)
    ...
    def is_wide_obstacle(self, collision_angles):
        ...
        return (
            max_segment >= self.wide_obstacle_angle_threshold
            or len(collision_angles) >= self.min_collision_points_for_wide
        )
    ...
    def obstacle_avoidance(
        self, lidar_ranges, lidar_intensities, range_down, range_up, distance_flew
    ):
        ...
        if self.state == 'ASCENDING':
            ...
            if (
                self.seen_obstacle >= self.seen_obstacle_min_to_overfly
                and not self.is_descending_permitted
            ):
            ...
            if (
                ...
                and self.is_descending_permitted
            ):
                self.is_extra = True
                self.extra_distance = self.safety_box_half
                return self.action('forward')
