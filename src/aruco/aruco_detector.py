...
import cv2
import numpy as np
from math import sqrt, atan2, pi, degrees, hypot, sin, cos
            ...
            if len(corners) > 0 and ids is not None:
                ...
                for marker_corner, marker_id in zip(corners, ids):
                    if marker_id in self.ids_to_detect:
                        ...
                        px_dist_from_c = hypot(marker_center_x - center_x, marker_center_y - center_y)
                        ...
                        theta = atan2(-1 * (marker_center_x - center_x), -1 * (marker_center_y - center_y))
                        ...
                        # pose_msg.pose.position.x = d_x / 100.0  # m
                        # pose_msg.pose.position.y = d_y / 100.0  # m
                        # pose_msg.pose.position.z = height_cm / 100.0  # m
                        ...
                        # тупо, но не хочу пока придумывать как делать свой кастомный формат
                        ...
                        # self.get_logger().info(f'Gamma = {self.marker_length/(97*sqrt(S / (800*600)))} ')
                        ...
                        if self.visualization_enabled:
                            ...
                            cv2.circle(vis_frame, (marker_center_x, marker_center_y), 4, (0, 0, 255), -1)
                            
                            cv2.line(vis_frame, (marker_center_x, marker_center_y), 
                                    (center_x, center_y), (0, 0, 255), 1)
                            ...
                            cv2.putText(
                                ...
                                ((center_x + marker_center_x) // 2, ((center_y) + marker_center_y) // 2),
                                cv2.FONT_HERSHEY_SIMPLEX,
                                0.5,
                                (0, 0, 255),
                                2,
