...
class CameraController(Node):
    def __init__(self):
        ...
        self.take_photo_sub = self.create_subscription(
            Int32, "/drone/photo/take_photo", self.take_photo_callback, 10
        )

        # Create publishers
        self.photo_status_pub = self.create_publisher(
            String, "/drone/photo/take_photo/status", 10
        )
        ...
        for attempt in range(self.connection_attempts):
            try:
                self.get_logger().info(
                    f"Attempting to connect to camera (attempt {attempt + 1}/{self.connection_attempts})..."
                )
    ...
    def check_camera_connection(self):
        ...
        if not self.camera_connected:
            ...
            try:
                ...
                result = subprocess.run(
                    ["gphoto2", "--summary"], capture_output=True, text=True, timeout=5
                )
    ...
    def take_photo_callback(self, msg):
        ...
        self.get_logger().info(
            f"Received request to take photo at location ID: {location_id}"
        )
    ...
    def get_camera_capabilities(self):
        ...
        try:
            result = subprocess.run(
                ["gphoto2", "--abilities"], capture_output=True, text=True, timeout=5
            )
