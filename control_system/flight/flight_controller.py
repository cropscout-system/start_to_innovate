...
from mavros_msgs.msg import GlobalPositionTarget, HomePosition, PositionTarget, State, StatusText
...
class TFL(Node):
    def __init__(self):
        ...
        self.battery_sub = self.create_subscription(
            BatteryState, '/mavros/battery', self.battery_callback, qos_best_effort_profile
        )
    ...
    def state_callback(self, msg):
        ...
        if msg and msg.system_status in (3, 4):  # MAV_STATE_STANDBY = 3, MAV_STATE_ACTIVE = 4
    ...
    def photo_taken_callback(self, msg: String):
        ...
        if (
            ...
            self.current_flight_phase = FlightPhase(
                name='taking_photo', status='reached', point=self.current_geo_point_processing
            )
    ...
    def flight_phases_observer(self):
        ...
        if self.current_state.armed and self.current_state.mode == 'GUIDED':
            ...
            if (
                ...
                self.current_flight_phase = FlightPhase(
                    name='descending_to_photo_altitude', point=self.current_geo_point_processing
                )
    ...
    def calculate_intermediate_coordinates(self, point_id):
        lat_intermediate = (
            self.current_global_position.latitude + self.geo_points.get(point_id).latitude
        ) / 2
        lon_intermediate = (
            self.current_global_position.longitude + self.geo_points.get(point_id).longitude
        ) / 2
        alt_intermediate = self.current_altitude_amsl + self.intermediate_altitude
        self.intermediate_coordinates = GeoPoint(
            latitude=lat_intermediate, longitude=lon_intermediate, altitude=alt_intermediate
        )
    ...
    def altitude_correction_by_rangefinder(self):
        if self.current_state.armed and self.current_state.mode == 'GUIDED':
            if (
                ...
                alt_delta = (
                    (self.current_altitude_rangefinder + 0.09) - self.target_photo_altitude
                )  # если коптер выше позиции, здесь положительная дельта
            ...
            if (
                ...
                alt_delta = (
                    (self.current_altitude_rangefinder + 0.09) - self.target_point_altitude
                )  # если коптер выше позиции, здесь положительная дельта
    ...
    def check_position_reached(self):
        ...
        if (
            ...
            if (
                self.calculate_distance(self.geo_points.get(self.current_geo_point_processing))
                <= 2.0
            ):
                self.current_flight_phase.status = 'reached'
