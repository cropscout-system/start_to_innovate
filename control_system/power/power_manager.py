...
class PowerManager(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "battery_critical_threshold", 10.8
        )  # Voltage for critical action
        self.declare_parameter(
            "check_interval", 10.0
        )  # Battery check interval in seconds
        self.declare_parameter(
            "rpi_battery_monitor", True
        )  # Whether to use Pi-specific monitoring
        self.declare_parameter(
            "publish_voltage", True
        )  # Whether to publish voltage as a topic
        self.declare_parameter(
            "simulate_battery", False
        )  # For testing without hardware
        self.declare_parameter(
            "simulate_voltage", 12.6
        )  # Starting voltage for simulation

        # Get parameters
        self.battery_warning_threshold = self.get_parameter(
            "battery_warning_threshold"
        ).value
        self.battery_critical_threshold = self.get_parameter(
        ...
        self.battery_check_timer = self.create_timer(
            self.check_interval, self.check_battery
        )
        ...
        if self.simulate_battery:
            ...
            self.get_logger().info(
                f"Starting with simulated battery voltage: {self.simulate_voltage}V"
            )

        self.get_logger().info("Power Manager node initialized")
        self.get_logger().info(
            f"Battery warning threshold: {self.battery_warning_threshold}V"
        )
        self.get_logger().info(
            f"Battery critical threshold: {self.battery_critical_threshold}V"
        )
    ...
    def check_battery(self):
        ...
        elif voltage > self.battery_warning_threshold and (
            self.warning_issued or self.return_home_issued
        ):
